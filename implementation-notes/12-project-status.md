...
- `[x]` Task `#324` / remote milestone `Practical MVP M4 Frontend Real Workflow`: admin setup wizard, module status, package release page, backend-backed package publish flow, mobile connection details, and pending sync status are implemented locally. PR pending.
...
- `[x]` Practical MVP M4 / `#324`: frontend setup wizard, module/plugin status page, package release page, real builder draft/publish API calls, dev package compile/activate flow, mobile manifest/download instructions, and smoke coverage are implemented locally. Backend sync result data remains M5 scope.
