...
- `[x]` Task `#323` / remote milestone `Practical MVP M3 Practical Builder APIs`: practical backend builder APIs now cover plugin/module status, validated form/app draft editing and publish, admin deployment package list/detail/compile/activate, and full backend integration through mobile manifest/download. PR pending.
...
- `[x]` Practical MVP M3 / `#323`: backend module/plugin status API, form/app draft editing and publish APIs, admin package compile/activate APIs, and the seed-to-mobile-package-fetch integration test are implemented locally. Frontend wizard UX and mobile runtime consumption remain M4/M6 scope.
