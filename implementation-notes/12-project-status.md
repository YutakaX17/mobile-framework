...
- `[~]` Task `#322` / remote milestone `Practical MVP M2 Real Backend Auth And Tenant Context`: backend session auth endpoints, tenant resolver, and initial builder permission checks are in progress; frontend fake-login replacement remains pending.
...
- `[~]` Practical MVP M2 / `#322`: backend Django session auth, CSRF bootstrap, tenant assignment APIs, `X-Tenant-Slug` resolver, and initial builder permission checks are in progress. Frontend auth replacement and complete API coverage remain pending.
