...
- `[x]` Task `#324` / PR `#332` / closed remote milestone `Practical MVP M4 Frontend Real Workflow`: admin setup wizard, module status, package release page, backend-backed package publish flow, mobile connection details, and pending sync status are merged.
...
- `[~]` Task `#326` / PR `#334` / remote milestone `Practical MVP M6 Mobile Practical Runtime`: transport-backed mobile package/sync client, key-value-backed package/token/outbox persistence, verified activation gating, practical render session, unsupported plugin/widget/action states, offline queueing, and sync receipt handling are implemented locally. PR pending.
...
- `[x]` Practical MVP M3 / `#323` / PR `#331`: backend module/plugin status API, form/app draft editing and publish APIs, admin package compile/activate APIs, and the seed-to-mobile-package-fetch integration test are merged.
- `[x]` Practical MVP M4 / `#324` / PR `#332`: frontend setup wizard, module/plugin status page, package release page, real builder draft/publish API calls, dev package compile/activate flow, mobile manifest/download instructions, and smoke coverage are merged. Backend sync result data remains M5 scope.
...
- `[~]` Practical MVP M6 / `#326` / PR `#334`: mobile practical runtime now has a transport-backed package/sync client, key-value-backed package/token/outbox persistence, verified package activation gating, render-state orchestration, unsupported plugin/widget/action failure states, offline submission queueing, and sync receipt handling. PR pending.
